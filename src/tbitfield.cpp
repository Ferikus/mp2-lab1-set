...
{
	TBitField maxBf = bf;
	TBitField minBf = *this;
	if (BitLen > bf.BitLen)
	{
		maxBf = *this;
		minBf = bf;
	}
	TBitField res(maxBf.GetLength());
	for (int i = 0; i < minBf.GetLength(); i++)
	{
		if (minBf.GetBit(i) == 0) res.ClrBit(i);
		else res.SetBit(i);
	}
	for (int i = 0; i < maxBf.GetLength(); i++)
		res.pMem[i] = res.pMem[i] | maxBf.pMem[i];
	return res;
}
